...
class DroneAvailabilityLogController():
    @staticmethod
    def calculate_datetime_limits(
            start_date: str,  # "2025-05-12"
            start_time: str,  # "00:17:07"  ── or ── "00:17:07.284475"
            time_thresh: int = 3  # hours before / after
    ):
        """
        Build a single datetime from a date-string + time-string, then
        return the ± `time_thresh` hour limits as both time-of-day and date strings.
        ...
        tuple[datetime, datetime, str, str]
            (start_time_limit_dt,
             end_time_limit_dt,
             start_date_limit_str,   # YYYY-MM-DD
             end_date_limit_str)     # YYYY-MM-DD
        """

        # 1) Parse each piece individually
        date_obj: date = datetime.strptime(start_date, "%Y-%m-%d").date()

        # `time.fromisoformat()` handles both "HH:MM:SS" and "HH:MM:SS.micro"
        time_obj: time = time.fromisoformat(start_time)

        # 2) Combine into one datetime
        start_dt: datetime = datetime.combine(date_obj, time_obj)

        # 3) Compute the ± threshold windows
        start_time_limit: datetime = start_dt - timedelta(hours=time_thresh)
        end_time_limit: datetime = start_dt + timedelta(hours=time_thresh)

        # 4) Extract the date parts you need as strings
        start_date_limit = start_time_limit.strftime("%Y-%m-%d")
        end_date_limit = end_time_limit.strftime("%Y-%m-%d")

        # ───────── debug prints (optional) ─────────
        print("Start Time:", start_dt.strftime("%H:%M:%S"))
    ...
    def insert_drone_availability_log(data):
        try:
            (start_time_limit,
             end_time_limit,
             start_date_limit,
             end_date_limit) = DroneAvailabilityLogController.calculate_datetime_limits(data['start_date'],data['start_time'])
